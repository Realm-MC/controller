...
// <<< IMPORTS ADICIONADOS PARA HOVER/CLICK EVENTS >>>
...
public class ServerConfigCommand implements CommandInterface {
    ...
    public ServerConfigCommand() {
        // O construtor está agora vazio. Os serviços serão carregados no primeiro uso.
    }

    // --- Métodos Lazy Getter ---

    public ServerInfoRepository getRepository() {
    ...
    public ServerRegistryService getServerRegistryService() {
        ...
        return this.serverRegistryService;
    }
    ...
    public Optional<SoundPlayer> getSoundPlayerOpt() {
        if (this.soundPlayerOpt == null) {
            this.soundPlayerOpt = ServiceRegistry.getInstance().getService(SoundPlayer.class);
        }
        return this.soundPlayerOpt;
    }
    ...
    // --- Handlers de Sub-comando ---

    private void handleCreate(CommandSource sender, String[] args, String label) {
        if (args.length < 3) {
            sendUsage(sender, label, "criar <id_servidor> <nome_de_exibicao>");
            return;
        }
        ...
        TaskScheduler.runAsync(() -> {
            try {
                ...
                ServerInfo newServer = ServerInfo.builder()
                        ...
                        .ip("0.0.0.0")
                        .port(25565)
                        .pterodactylId("NOT_SET")
                        .type(ServerType.PERSISTENT)
                ...
                handleCommandError(sender, "criar servidor", e);
            }
        });
    }
    ...
    private void handleDelete(CommandSource sender, String[] args, String label) {
        if (args.length < 2) {
            sendUsage(sender, label, "deletar <id_servidor>");
            return;
        }
        ...
        TaskScheduler.runAsync(() -> {
            try {
                ...
                if (serverOpt.isEmpty()) {
                    Messages.send(sender, Message.of(MessageKey.SCONFIG_NOT_FOUND).with("id", serverId));
                    playSound(sender, SoundKeys.ERROR);
                    return;
                }
                ...
                handleCommandError(sender, "deletar servidor", e);
            }
        });
    }
    ...
    private void handleSetInt(CommandSource sender, String[] args, String label, String property) {
        if (args.length < 3) {
            sendUsage(sender, label, args[0] + " <id_servidor> <numero>");
            return;
        }
        String serverId = args[1];
        int value;
        try {
            ...
            Messages.send(sender, Message.of(MessageKey.SCONFIG_INVALID_NUMBER).with("value", args[2]));
            playSound(sender, SoundKeys.ERROR);
            return;
        }
        ...
        findAndModifyServer(sender, serverId, "set " + property, server -> {
            ...
            Messages.send(sender, Message.of(MessageKey.SCONFIG_PROP_UPDATED)
                    .with("prop", property)
                    .with("id", serverId)
                    .with("value", value));
    ...
    private void handleSetString(CommandSource sender, String[] args, String label, String property) {
        if (args.length < 3) {
            sendUsage(sender, label, args[0] + " <id_servidor> <valor>");
            return;
        }
        String serverId = args[1];
        ...
        findAndModifyServer(sender, serverId, "set " + property, server -> {
            ...
            Messages.send(sender, Message.of(MessageKey.SCONFIG_PROP_UPDATED)
                    ...
                    .with("value", value));
            playSound(sender, SoundKeys.SETTING_UPDATE);
            if (property.equals("ip") || property.equals("displayname")) {
                getServerRegistryService().initialize();
            }
        });
    }
    ...
    private void handleSetMinGroup(CommandSource sender, String[] args, String label) {
        if (args.length < 3) {
            sendUsage(sender, label, "mingroup <id_servidor> <nome_grupo>");
            return;
        }
        String serverId = args[1];
        ...
        findAndModifyServer(sender, serverId, "set mingroup",
                server -> server.setMinGroup(groupName),
                () -> {
                    Messages.send(sender, Message.of(MessageKey.SCONFIG_PROP_UPDATED)
                            .with("prop", "mingroup")
                            .with("id", serverId)
                            .with("value", groupName));
                    playSound(sender, SoundKeys.SETTING_UPDATE);
                });
    }
    ...
    private void handleSetType(CommandSource sender, String[] args, String label) {
        if (args.length < 3) {
            sendUsage(sender, label, "type <id_servidor> <tipo>");
            return;
        }
        String serverId = args[1];
        ...
        try {
            type = ServerType.valueOf(typeName);
        } catch (IllegalArgumentException e) {
            Messages.send(sender, Message.of(MessageKey.SCONFIG_INVALID_TYPE)
                    .with("types", Arrays.toString(ServerType.values())));
            playSound(sender, SoundKeys.ERROR);
            return;
        }

        findAndModifyServer(sender, serverId, "set type",
                server -> server.setType(type),
                () -> {
                    Messages.send(sender, Message.of(MessageKey.SCONFIG_PROP_UPDATED)
                            .with("prop", "type")
                            .with("id", serverId)
                            .with("value", type.name()));
                    playSound(sender, SoundKeys.SETTING_UPDATE);
                    getServerRegistryService().initialize();
                });
    }
    ...
    private void handleInfo(CommandSource sender, String[] args, String label) {
        if (args.length < 2) {
            sendUsage(sender, label, "info <id_servidor>");
            return;
        }
        String serverId = args[1];
        ...
        TaskScheduler.runAsync(() -> {
            try {
                ...
                TaskScheduler.runSync(() -> {
                    // <<< CORREÇÃO: Traduz o Status usando as novas chaves >>>
                    ...
                    Messages.send(sender, Message.of(MessageKey.COMMON_INFO_LINE).with("key", Messages.translate(MessageKey.SCONFIG_INFO_KEY_STATUS, locale)).with("value", translatedStatus)); // Usa o status traduzido
                ...
                handleCommandError(sender, "info servidor", e);
            }
        });
    }
        ...
        TaskScheduler.runAsync(() -> {
            try {
                ...
                servers.sort(Comparator
                        .comparing(ServerInfo::getType)
                        .thenComparing(ServerInfo::getName)
                );

                TaskScheduler.runSync(() -> {
                    Messages.send(sender, Message.of(MessageKey.SCONFIG_LIST_HEADER).with("count", servers.size()));
                    if (servers.isEmpty()) {
                        Messages.send(sender, MessageKey.SCONFIG_LIST_EMPTY);
                    } else {
                        for (ServerInfo s : servers) {

                            // <<< INÍCIO DA CORREÇÃO (Status Traduzido e Hover Lógico) >>>

                            // 1. Traduz o status e define a cor
                            MessageKey statusKey;
                            String statusColor;
                            switch (s.getStatus()) {
                                case ONLINE:
                                    statusKey = MessageKey.SCONFIG_INFO_STATUS_ONLINE;
                                    statusColor = "<green>";
                                    break;
                                case STARTING:
                                    statusKey = MessageKey.SCONFIG_INFO_STATUS_STARTING;
                                    statusColor = "<yellow>";
                                    break;
                                case STOPPING:
                                    statusKey = MessageKey.SCONFIG_INFO_STATUS_STOPPING;
                                    statusColor = "<dark_red>";
                                    break;
                                case OFFLINE:
                                default:
                                    statusKey = MessageKey.SCONFIG_INFO_STATUS_OFFLINE;
                                    statusColor = "<red>";
                                    break;
                            }
                            String translatedStatus = Messages.translate(statusKey, locale);

                            // 2. Constrói os placeholders
                            Map<String, Object> placeholders = new HashMap<>();
                            placeholders.put("name", s.getName());
                            placeholders.put("displayName", s.getDisplayName());
                            placeholders.put("type", s.getType().name());
                            placeholders.put("status_color", statusColor); // Cor (para o hover E a linha)
                            placeholders.put("status", translatedStatus); // Texto traduzido (para a linha)
                            ...
                            // 3. Constrói o texto do Hover
                            // O status no hover usará o texto traduzido (sem cor)
                            placeholders.put("status", translatedStatus.replaceAll("<[^>]*>", "")); // Remove tags de cor para o hover
                            ...
                            // 4. Seleciona a AÇÃO correta do Hover (ONLINE, STARTING, OFFLINE)
                            MessageKey hoverActionKey;
                            if (s.getStatus() == ServerStatus.ONLINE) {
                                hoverActionKey = MessageKey.SCONFIG_LIST_LINE_HOVER_ACTION_ONLINE;
                            } else if (s.getStatus() == ServerStatus.STARTING) {
                                hoverActionKey = MessageKey.SCONFIG_LIST_LINE_HOVER_ACTION_STARTING;
                            } else {
                                hoverActionKey = MessageKey.SCONFIG_LIST_LINE_HOVER_ACTION_OFFLINE;
                            }
                            String hoverAction = Messages.translate(hoverActionKey, locale);

                            Component hoverComponent = miniMessage.deserialize(hoverHeader + "\n" + hoverDetails + "\n" + hoverAction);

                            // 5. Constrói a linha principal (que usa o 'status' traduzido e 'displayName')
                            String lineText = Messages.translate(Message.of(MessageKey.SCONFIG_LIST_LINE).with(placeholders), locale);
                            Component lineComponent = miniMessage.deserialize(lineText);

                            // 6. Adiciona o Hover e Click
                            lineComponent = lineComponent.hoverEvent(
                                    HoverEvent.showText(hoverComponent)
                            );

                            if (s.getStatus() == ServerStatus.ONLINE && sender instanceof Player) {
                                lineComponent = lineComponent.clickEvent(
                                        ClickEvent.runCommand("/server " + s.getName())
                                );
                            }

                            // 7. Envia o Componente
                ...
                handleCommandError(sender, "listar servidores", e);
            }
        });
    }
    ...
    private void handleReload(CommandSource sender) {
        TaskScheduler.runAsync(() -> {
            try {
                ...
                handleCommandError(sender, "reload servidores", e);
            }
        });
    }
    ...
    private void findAndModifyServer(CommandSource sender, String serverName, String actionName,
                                     Consumer<ServerInfo> modification, Runnable onMainThreadSuccess) {

        TaskScheduler.runAsync(() -> {
            try {
                Optional<ServerInfo> serverOpt = getRepository().findByName(serverName);
                if (serverOpt.isEmpty()) {
                    Messages.send(sender, Message.of(MessageKey.SCONFIG_NOT_FOUND).with("id", serverName));
                    playSound(sender, SoundKeys.ERROR);
                    return;
                }
                ...
                handleCommandError(sender, actionName, e);
            }
        });
    }
    ...
    private void showHelp(CommandSource sender, String label) {
        ...
        for (Map.Entry<String, MessageKey> entry : helpMessages.entrySet()) {
            Messages.send(sender, Message.of(MessageKey.COMMON_HELP_LINE)
                    .with("usage", entry.getKey().replace("{label}", label))
    ...
    public List<String> tabComplete(CommandSource sender, String[] args) {
        ...
        switch (args.length) {
            case 1: // /sconfig <subcomando>
                suggestions.addAll(Arrays.asList(
                        "criar", "deletar", "maxplayer", "maxplayervip", "ip", "port",
                        "mingroup", "pteroid", "setdisplayname", "type", "info", "list", "reload", "help"
                ));
                break;

            case 2: // /sconfig <subcomando> <id_servidor>
                String subCmd = args[0].toLowerCase();

                // Comandos que operam em um ID de servidor
                if (Arrays.asList("deletar", "maxplayer", "maxplayervip", "ip", "port", "mingroup",
                        "pteroid", "setdisplayname", "type", "info").contains(subCmd)) {

                    Set<String> serverNames = getProxyServer().getAllServers().stream()
                            .map(RegisteredServer::getServerInfo)
                            .map(com.velocitypowered.api.proxy.server.ServerInfo::getName)
                            .collect(Collectors.toSet());

                    // Busca IDs no DB para inclusão (útil para IDs de servidores offline/não registrados)
                    try {
                        getRepository().collection().find().forEach(serverInfo -> serverNames.add(serverInfo.getName()));
                    } catch (Exception e) {
                        getLogger().warning("Falha ao buscar nomes do DB para tab-complete: " + e.getMessage());
                    }
                    suggestions.addAll(serverNames);
                }
                break;

            case 3: // /sconfig <subcomando> <id_servidor> <valor>
                String subCmd3 = args[0].toLowerCase();

                if (subCmd3.equals("type")) {
                    // Sugere ServerTypes
                    suggestions.addAll(Stream.of(ServerType.values()).map(Enum::name).collect(Collectors.toList()));
                } else if (subCmd3.equals("mingroup")) {
                    // Sugere nomes de Roles
                    suggestions.addAll(getRoleService().getAllCachedRoles().stream()
                            .map(Role::getName)
                            .collect(Collectors.toList()));
                } else if (subCmd3.equals("maxplayer") || subCmd3.equals("maxplayervip") || subCmd3.equals("port")) {
                    // Sugere valores numéricos comuns
                    suggestions.addAll(Arrays.asList("20", "50", "100", "25565", "8080"));
                } else if (subCmd3.equals("ip")) {
                    // Sugere IPs comuns
                    suggestions.addAll(Arrays.asList("127.0.0.1", "10.0.0.1", "192.168.1.1"));
                }
                break;
        }
        ...
        return suggestions.stream()
                ...
                .map(s -> suggestions.stream().filter(orig -> orig.toLowerCase().equals(s)).findFirst().orElse(s)) // Retém o casing original
                .distinct()
                .sorted() // <<< CORREÇÃO: Adicionado o ponto (.)
                .collect(Collectors.toList());
    }
}