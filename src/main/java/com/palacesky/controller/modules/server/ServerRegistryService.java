...
import com.palacesky.controller.modules.server.data.*;
    ...
    private void setupDefaultServers() {
        try {
            for (DefaultServer defaultServer : DefaultServer.values()) {
                ...
                com.palacesky.controller.modules.server.data.ServerInfo defaultInfo = defaultServer.toServerInfo();
                if (existingOpt.isEmpty()) { repository.save(defaultInfo); }
                else {
                    com.palacesky.controller.modules.server.data.ServerInfo existing = existingOpt.get();
