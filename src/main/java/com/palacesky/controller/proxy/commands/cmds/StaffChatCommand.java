...
public class StaffChatCommand implements CommandInterface {
    ...
    public void execute(CommandSource sender, String label, String[] args) {
        ...
        if (args.length == 0) {
            Messages.send(sender, Message.of(MessageKey.COMMON_USAGE).with("usage", "/" + label + " <mensagem>"));
            playSound(sender, SoundKeys.USAGE_ERROR);
            return;
        }

        Optional<ChatService> chatServiceOpt = ServiceRegistry.getInstance().getService(ChatService.class);
        if (chatServiceOpt.isEmpty()) {
            Messages.send(sender, "&cErro: Sistema de Chat não está carregado. Avise um administrador.");
            return;
        }
        ChatService chatService = chatServiceOpt.get();

        boolean isStaffChatEnabled = preferencesService.getCachedStaffChatEnabled(player.getUniqueId()).orElse(true);
        ...
        String formattedName = NicknameFormatter.getGroupFormattedName(player.getUniqueId());
